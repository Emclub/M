...
settings = {
    ...
    "welcome":"ยินดีต้อนรับสมาชิกใหม่\n\nตั้งข้อความต้อนรับ",
    "kick":"บ๊าย บาย!!..",
    "bye":"รีบไปไหนคร้ายบ",
    "Respontag":"คร้าบบ ว่างัยเอ๋ยย",
    ...
    "message":"บัญชีนี้ถูกป้องกันโดย เอ็มBot Self  ระบบได้ทำการบล็อคคุณอัตโนมัติ!..",
    "comment":"""เอ็มBot Self:
╔══════════════┓
╠     🔱เอ็ม Bot Self 🔱
╚══════════════┛""",
    "userAgent": [
                         ...
                         if targets == []:
                             ...
                             for target in targets:
                                 try:
                                     ...
                                     line.sendText(msg.to,"ทำการเชิญคนนี้สำเร็จแล้ว : \n👉" + _name)
...
#==============================================================================#
        ...
        if op.type == 19:
           ...
           if settings["Nk"] == True:
             ...
             dan = line.getContact(op.param2)
             tgb = line.getGroup(op.param1)
             line.sendMessage(op.param1,str(settings["kick"]) + "\nยืนไว้อาลัยครึ่งวินาที {},บ๊าย บาย...".format(str(dan.displayName)))
             line.sendContact(op.param1, op.param2)
             line.sendMessage(op.param1,"สเตตัส\n{}".format(str(dan.statusMessage)))
             line.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
        if op.type == 15:
           ...
           if settings["Lv"] == True:
             ...
             dan = line.getContact(op.param2)
             tgb = line.getGroup(op.param1)
             line.sendMessage(op.param1,str(settings["bye"]) + "\n {}, ไปไหน {} \nลาก่อน".format(str(dan.displayName),str(tgb.name)))
             line.sendContact(op.param1, op.param2)
             line.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
        if op.type == 55:
            try:
